...
function install(Lobby) {
  ...
  function makeMethodLocals(
    ...
    const methodLocals = {
      ...
      [StopStatusSymbol]: STOP_STATUS_NORMAL,
      return(value) {
        methodLocals[StopStatusSymbol] = STOP_STATUS_RETURN;
        return value;
      },
      break() {
        methodLocals[StopStatusSymbol] = STOP_STATUS_BREAK;
      },
      continue() {
        methodLocals[StopStatusSymbol] = STOP_STATUS_CONTINUE;
      },
    };
    for (const n of methodArgNames) {
      methodLocals[n] = null;
    }
    // TODO try to use null as localsTarget
    Object.setPrototypeOf(methodLocals, localsTarget);
    return methodLocals;
  }
  ...
  function evalMessage(Lobby, firstMsg, context, locals) {
    ...
    while (msg) {
      ...
      localsTarget = findSlotTarget(Lobby, target);
      cursor = findSlot(Lobby, msg, localsTarget, locals);
      ...
      if (cursor instanceof Promise) {
        return continueEvalMessageAsync(Lobby, msg, context, locals, cursor);
      }
...
function findSlotTarget(Lobby, target) {
