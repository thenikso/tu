  ...
  do {
    ...
    if (msg.isLiteral) {
      switch (typeof msg.name) {
        case 'string':
          cursor = Object.create(Lobby.Str);
          cursor.value = msg.name;
          break;
        case 'number':
          cursor = Object.create(Lobby.Num);
          cursor.value = msg.name;
          break;
        case 'boolean':
          cursor = Object.create(Lobby.Bool);
          cursor.value = msg.name;
          break;
        default:
          throw new Error(`Unknown literal type: ${typeof msg.name}`);
      }
      target = cursor;
      msg = msg.next;
      continue;
      ...
      if (typeof methodArgs === 'function') {
        ...
        if (typeof l !== 'undefined') {
          ...
          for (; i < l; i++) {
            ...
            localArgs[i] = evalMessage(Lobby, msg.arguments[i], sender);
          }
          ...
          for (l = msg.arguments?.length; i < l; i++) {
            ...
            localArgs[i] = bodyMsg
              ? function () {
                  return evalMessage(Lobby, bodyMsg, this);
                }
              : returnNull;
          }
        }
