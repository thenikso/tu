...
function install(Lobby) {
  ...
  Object.defineProperty(
    ...
    'message',
    dValue(
      method(function Receiver_message(msg) {
        if (msg?.$message) {
          return msg.$message;
        }
        // Not using `Message.clone()` or we get an infinite loop
  ...
  Object.defineProperty(
    ...
    'doMessage',
    dValue(
      Receiver_method('msg', function Receiver_doMessage(msg) {
  ...
  function Receiver_method(...args) {
    ...
    const argNames = args.slice().map((arg) => {
      if (typeof arg === 'string') {
        return arg;
      }
      return arg.$message?.name;
    });
    ...
    function methodFn(...argValues) {
      const result = bodyFn.apply(this, argValues);
      return result;
    }
    function Method_apply(thisArg, argsArray, msg, sender) {
      // If `this` is a literal value (e.g. a number), we "proxy" locals to the
      // appropriate Lobby object.
      let localsTarget = thisArg;
      ...
      for (let i = 0; i < argNames.length; i++) {
        locals[argNames[i]] = argsArray[i] ?? null;
      }

      const result = this.apply(locals, argsArray);
      // const result = bodyFn.$message.doInContext(thisArg, locals);
    ...
    Object.defineProperties(methodFn, {
      ...
      toString: {
        value: () =>
          `method(${[...argNames, bodyFn.$message?.toString()].join(', ')})`,
      },
    });
    return methodFn;
  }
  ...
  do {
    ...
    if (typeof cursor === 'function') {
      ...
      if (MethodApplySymbol in cursor) {
        cursor = cursor[MethodApplySymbol](target, localArgs, msg, sender);
      } else {
