...
export function method(...args) {
  ...
  function Method_apply(target, msg, sender, Lobby) {
    ...
    const locals = Object.create(localsTarget, {
      ...
      [StopStatusSymbol]: {
        value: STOP_STATUS_NORMAL,
        writable: true,
      },
      return: {
        value: function Method_return(value) {
          locals[StopStatusSymbol] = STOP_STATUS_RETURN;
          return value;
        },
      },
      break: {
        value: function Method_break() {
          locals[StopStatusSymbol] = STOP_STATUS_BREAK;
        },
      },
      continue: {
        value: function Method_continue() {
          locals[StopStatusSymbol] = STOP_STATUS_CONTINUE;
        },
      },
    });
    let promiseLocalsArgs = null;
    if (methodArgsLen > 0) {
      if (msg.arguments.length < methodArgsLen) {
        throw new EvalError(
          `Not enough arguments for method "${msg.name}". Expected ${methodArgsLen}, got ${msg.arguments.length}.`,
          msg,
        );
      }
      ...
      for (let i = 0; i < methodArgsLen; i++) {
        ...
        localArg = evalMessage(Lobby, msg.arguments[i], argSender, argLocals);
        if (localArg instanceof Promise) {
          const localArgName = methodArgs[i];
          promiseLocalsArgs.push(
            localArg.then((localArgValue) => {
              locals[localArgName] = localArgValue;
            }),
          );
        } else {
          locals[methodArgs[i]] = localArg;
        }
      }
    }

    // Resolve all promises in the arguments
    if (promiseLocalsArgs && promiseLocalsArgs.length > 0) {
      return Promise.all(promiseLocalsArgs).then(() =>
        this.call(target, locals),
      );
    }

    return this.call(target, locals);
  }
...
function install(Lobby) {
  ...
  function Receiver_method(locals) {
    ...
    function methodFn(methodLocals) {
      ...
      const result = evalMessage(Lobby, bodyMsg, this, methodLocals);
      return result;
    }
    ...
    Object.defineProperty(methodFn, 'toJs', {
      value: () => (...args) => {
        // TODO implement me, or even better make methods compatible with
        //      javascript methods
  ...
  do {
    ...
    if (typeof cursor === 'function') {
      if (MethodApplySymbol in cursor) {
        ...
        const localArgs = (msg.arguments ?? []).map((argMsg) => {
        ...
        if (hasAsyncArgs) {
          return Promise.all(localArgs).then(async (args) => {
            cursor = await cursor.apply(target, args);
            return evalMessageAsync(
              Lobby,
              msg,
              context,
              locals,
    ...
    if (cursor instanceof Promise) {
      return evalMessageAsync(Lobby, msg, context, locals, cursor, target);
    }

    // return statement
    if (locals?.[StopStatusSymbol] > 0) {
      return cursor;
    }
