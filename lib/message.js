...
function install(Lobby) {
  ...
  function Receiver_method(locals) {
    ...
    const methodString = `method(${[...argNames, bodyMsg?.toString()].join(
      ', ',
    )})`;
      ...
      value:
        ...
        (...args) => {
          if (argNames.length > args.length) {
            throw new Error(`Not enough arguments to method "${methodString}"`);
          }
  ...
  function Receiver_block(locals) {
    ...
    function blockFn(blockLocals) {
      if (!bodyMsg) {
        return null;
      }
      const result = evalMessage(Lobby, bodyMsg, locals, blockLocals, true);
      return result;
    }
    method(...argNames, blockFn);
    const blockString = `block(${[...argNames, bodyMsg?.toString()].join(
      ', ',
    )})`;
    Object.defineProperty(blockFn, 'toString', {
      value: () => blockString,
    });
    Object.defineProperty(blockFn, 'toJs', {
      value:
        () =>
        (...args) => {
          if (argNames.length > args.length) {
            throw new Error(`Not enough arguments to block "${blockString}"`);
          }
          // TODO better construction of `locals` to include `self` and `call`
          // like in `Method_apply`
          const locals = Object.create(null);
          ...
          // TODO have `Lobby.Envine.target` to be the current target
          // @ts-ignore-next-line
          return blockFn.call(Lobby, locals);
        },
    });
    return blockFn;
  }
   ...
   * TODO understand better what `relayMethodStopStatus` does and name it better
