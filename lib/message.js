    ...
    const locals = Object.create(localsTarget, {
      ...
      return: {
        value: function Method_return(value) {
          stopStatus.isReturn = true;
          return value;
        },
      },
      break: {
        value: function Method_break() {
          stopStatus.isBreak = true;
        },
      },
      continue: {
        value: function Method_continue() {
          stopStatus.isContinue = true;
        },
      },
    });
...
function evalMessage(Lobby, firstMsg, context, locals) {
  ...
  do {
    ...
    // return statement
    if (
      locals?.stopStatus?.isReturn ||
      locals?.stopStatus?.isBreak ||
      locals?.stopStatus?.isContinue
    ) {
      return cursor;
    }
