...
export function primitivesInstaller(method) {
  return {
    install(Lobby) {
       ...
       */
      async function Bool_ifTrue(locals) {
        if (this === true) {
       ...
       * @returns {Promise<boolean>}
       */
      async function Bool_ifFalse(locals) {
        if (this === false) {
          await locals.call.evalArgAt(0);
        }
        return this;
       ...
       * @returns {Promise<boolean>}
       */
      async function Bool_elseif(locals) {
        if (this === false) {
          return (await locals.call.evalArgAt(0)) ?? false;
        }
        return this;
      }
      method(Bool_elseif);
       ...
       * @returns {Promise<any[]>}
       */
      async function List_select(locals) {
        const { body } = getIndexItemBody(locals);
        const result = [];
        for (let i = 0, l = this.length; i < l; i++) {
          if (await body(this[i], i)) {
            result.push(this[i]);
          }
        }
        return result;
       ...
       * @returns {Promise<any>}
       */
      async function List_detect(locals) {
        const { body } = getIndexItemBody(locals);
        for (let i = 0, l = this.length; i < l; i++) {
          const item = this[i];
          const result = await body(item, i);
          if (result !== null) {
            return item;
          }
        }
       ...
       * @returns {Promise<any[]>}
       */
      async function List_map(locals) {
        const { body } = getIndexItemBody(locals);
        const result = [];
        for (let i = 0, l = this.length; i < l; i++) {
          result.push(await body(this[i], i));
        }
        return result;
      }
      method(List_map);
       ...
       * @returns {Promise<any[]>}
       */
      async function List_foreach(locals) {
        const { body } = getIndexItemBody(locals);
        for (let i = 0, l = this.length; i < l; i++) {
          await body(this[i], i);
        }
       ...
       * @returns {Promise<Map>}
       */
      async function Map_foreach(locals) {
        const { body } = getIndexItemBody(locals);
        for (const [key, value] of this.entries()) {
          await body(value, key);
        }
        return this;
      }
      method(Map_foreach);
