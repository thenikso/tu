...
const RECEIVER_DESCRIPTORS = {
  ...
  asSimpleString: dValue(function Receiver_asSimpleString() {
    return (
      this.type +
      '_' +
      (typeof this.uniqueId === 'function' ? this.uniqueId() : this.uniqueId)
    );
  }),
  uniqueId: dValue(function Receiver_uniqueId() {
    if (Object.hasOwnProperty.call(this, 'uniqueId')) {
      return this.uniqueId;
    }
    const id =
      '0x' +
      Math.abs(Date.now() ^ (Math.random() * 10000000000000)).toString(16);
