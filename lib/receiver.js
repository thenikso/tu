    ...
    install(nil) {
      ...
      const Exception = createReceiver(rootReceiver, {
        ...
        raise: dValue(function Exception_raise(message) {
          ...
          method(
            'exceptionProto',
            function Exception_catch(exceptionProto, body) {
              if (this instanceof exceptionProto) {
                body();
              }
              return this;
            },
      ...
      Object.defineProperties(rootReceiver, {
        ...
        try: dValue(
          method(function Receiver_try(body) {
            try {
              body();
              return null;
            } catch (error) {
...
export function createRootReceiver(method = (x) => x) {
  const Receiver = Object.create(null, {
    ...
    for: dValue(
      method(function Receiver_for(i, start, end, body) {
        if (arguments.length !== 4) {
          throw new Error(`message 'for' requires 4 arguments`);
        }
        const iName = i.$message?.name;
        if (!iName) {
        ...
        if (typeof endIndex !== 'number') {
          throw new Error(`message 'for' requires a number for the end index`);
        }
        const locals = {
          [iName]: startIndex,
        };
    ...
    do: dValue(
      method(
         ...
         * @this {import('./types').Receiver}
         * @param {Function} body
         * @returns {import('./types').Receiver}
         */
        function Receiver_do(body) {
          body?.call(this);
          return this;
        },
      ),
