...
import { dValue, dGet, dType, TypeSymbol, lock } from './util.js';
  ...
  return {
    ...
    install(nil) {
      ...
      const Exception = createReceiver(rootReceiver, {
        ...
        raise: dValue(function Exception_raise(message) {
          ...
          method(
             ...
             */
            async function Exception_catch(locals) {
              if (this.hasProto(locals.exceptionProto)) {
                await locals.call.evalArgAt(1);
              }
              return this.self;
            },
          ),
      ...
      Object.defineProperties(rootReceiver, {
        exception: dValue(async function Receiver_exception(error) {
          const exception = await Exception.clone();
          exception.error = error;
          return exception;
        }),
        try: dValue(
          method(async function Receiver_try(locals) {
            try {
              await locals.call.evalArgAt(0);
              return null;
            } catch (error) {
  ...
  const Receiver = Object.create(null, {
    ...
    for: dValue(
      method(
         ...
         */
        async function Receiver_for(locals) {
          ...
          const startIndex = await locals.call.evalArgAt(1);
          if (typeof startIndex !== 'number') {
            throw new Error(
              `message 'for' requires a number for the start index`,
            );
          }
          const endIndex = await locals.call.evalArgAt(2);
          ...
          if (locals.call.message.arguments.length > 4) {
            step = await locals.call.evalArgAt(3);
          ...
          if (startIndex === endIndex) {
            await bodyMsg.doInContext(bodyCtx, newLocals);
            return endIndex;
          }
          if (startIndex < endIndex) {
            if (step <= 0) {
            ...
            for (let i = startIndex; i <= endIndex; i += step) {
              newLocals[iName] = i;
              await bodyMsg.doInContext(bodyCtx, newLocals);
              if (newLocals.stopStatus?.isBreak) {
                break;
              }
              if (newLocals.stopStatus?.isContinue) {
                newLocals.stopStatus.isContinue = false;
                continue;
              }
            }
    ...
    if: dValue(
      method(
         ...
         */
        async function Receiver_if(locals) {
          if (locals.condition) {
    ...
    do: dValue(
      method(
         ...
         * @this {import('./types').Receiver}
         * @param {import('./types').Locals} locals
         * @returns {Promise<import('./types').Receiver>}
         */
        async function Receiver_do(locals) {
          const bodyMsg = locals.call.message.arguments[0];
          if (bodyMsg) {
            await bodyMsg.doInContext(this, locals);
          }
          return this;
        },
      ),
