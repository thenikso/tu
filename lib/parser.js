...
tuSemantics.addOperation('toMessage(Lobby, assigns, infixes)', {
  ...
  number_exp(sign, digit1, _dot, digit2, e, esign, digit3) {
    const { Lobby } = this.args;
    const msg = Lobby.message();
    msg.name = parseFloat(
      `${sign.sourceString}${digit1.sourceString || '0'}.${
        digit2.sourceString
      }${
        e.sourceString
  ...
  number_dec(sign, digits) {
    const { Lobby } = this.args;
    const msg = Lobby.message();
    msg.name = parseInt(sign.sourceString + digits.sourceString, 10);
    msg.isLiteral = true;
