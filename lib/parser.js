...
const grammar = ohm.grammar(String.raw`Tu {
  ...
  exp
    = exp? sp "{" spaces listOf<exps, comma> spaces "}" -- curlyBrackets
    | exp? sp "[" spaces listOf<exps, comma> spaces "]" -- squareBrackets
    | exp sp message                                    -- multiMessage
    | message                                           -- singleMessage
    | "(" spaces exp spaces ")"                         -- parentheses

  message
    = symbol sp arguments -- args
    | symbol              -- single

  arguments
    = "(" spaces listOf<exps, comma> sp terminator? spaces ")"
...
semantics.addOperation('toMessage(Lobby, assigns, infixes)', {
  Program(exps, _1) {
    ...
    for (const [assignMethod, messages] of Object.entries(
      ...
      for (const assignOp of messages) {
        ...
        while (assignEnd && !assignEnd?.isTerminator) {
  ...
  exp_curlyBrackets(exp, s1, lb, s2, exps, s3, rb) {
    throw new Error('Brackets not implemented');
  },
  exp_squareBrackets(exp, s1, lb, s2, exps, s3, rb) {
    throw new Error('Brackets not implemented');
  },
  message_single(symbol) {
    const { Lobby, assigns, infixes } = this.args;
  ...
  arguments(bl, s1, args, s2, term, s3, br) {
    const { Lobby, assigns, infixes } = this.args;

    const msgs = args
      .asIteration()
      .children.map((exps) => exps.toMessage(Lobby, assigns, infixes));
    if (term.sourceString && msgs.length > 0) {
      const termMsg = Lobby.message();
      termMsg.name = ';'; //term.sourceString;
      termMsg.isTerminator = true;
      msgs[msgs.length - 1].last.setNext(termMsg);
    }
    return msgs;
  },
  symbol(symbol) {
