...
const OPERATOR_TABLE_DESCRIPTORS = {
  ...
  assignOperators: {
    ...
    value: new Map([
      ...
      [':=', 'setSlot'],
      ['=', 'updateSlot'],
      // TODO implement this. Subscribe to slot used in body and inform
      // subscribers when it changes in `setSlot`/`updateSlot`.
      // also if `updateSlot` on a reactiveSlot, unsubscribe it.
      ['$=', 'reactiveSlot'],
    ]),
  },
};
...
tuSemantics.addOperation('toMessage(Lobby, assigns, infixes)', {
  ...
  number_exp(digit1, _dot, digit2, e, esign, digit3) {
    ...
    const msg = Lobby.message();
    msg.name = parseFloat(
      `${digit1.sourceString || '0'}.${digit2.sourceString}${
        e.sourceString
