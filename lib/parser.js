...
const tuGrammar = grammar(String.raw`Tu {
  ...
  expression
    = (message | sctpad)+
  message
    = scpad* symbol scpad* arguments?  -- single
    | brackets                         -- group
  brackets
    = "(" expression ")"               -- round
  ...
  comment
    = slashStarComment | slashSlashComment | hashComment
  slashStarComment
    = "/*" (~"/*" any)* "*/"
  slashSlashComment
    = "//" (~"\n" any)* "\n"
...
tuSemantics.addOperation('toMessage(Lobby, assigns, infixes)', {
  _iter(...children) {
    ...
    for (const child of children) {
      ...
      if (!msg) {
        ...
        if (newMsg.isTerminator && lastMsg.isTerminator) {
  ...
  message_single(_pad1, symbol, _pad2, args) {
    const { Lobby, assigns, infixes } = this.args;
  ...
  arguments(_open, _pad1, args, _close) {
    ...
    if (msgs.length > 0) {
      ...
      if (last.isTerminator && last.name !== ';') {
  ...
  quote(_open, chars, _close) {
    const { Lobby } = this.args;
    const msg = Lobby.message();
    // TODO if triple quote, do not unescape
  ...
  number_exp(digit1, _dot, digit2, e, sign, digit3) {
    const { Lobby } = this.args;
    const msg = Lobby.message();
    msg.name = parseFloat(
      `${digit1.sourceString || '0'}.${digit2.sourceString}` + e.sourceString
        ? `e${sign.sourceString || ''}${digit3.sourceString}`
        : '',
    );
    msg.isLiteral = true;
    return msg;
